...
// runtime (in this case, it will return s string in the format "<runtime> mins").)
...
func (r *Runtime) UnmarshalJSON(jsonValue []byte) error {
	...
	// We expect that incoming JSON value will be a strong in the format
	...
	// If is isn't, we return the ErrInvalidRuntimeFormat error again.
	if len(parts) != 2 || parts[1] != "mins" {
		return ErrInvalidRuntimeFormat
	}

	// Otherwise, parse the string containing the number into an int32. Again if this
	// fails return the ErrInvalidRuntimeEormat error.
	...
	// use the * operator to dereference the receiver (which is the pointer to a runtime
